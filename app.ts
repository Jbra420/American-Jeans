// Declaración para el SDK de Firebase cargado en el ámbito global
...
class Showroom {
    ...
    private currentStatus: string = 'Todos';
    private adminPasswordVal: string = 'american123'; // Contraseña por defecto
    private isDbOnline: boolean = false; // Indica si Firebase está conectado
    private db: any = null; // Instancia de Firestore

    // Elementos del Catálogo
    private gridElement!: HTMLElement;
    private genderFilterContainer!: HTMLElement;
    private statusFilterContainer!: HTMLElement;
    ...
    private dbStatusIndicator!: HTMLElement;
    ...
    private formStatusSelect!: HTMLSelectElement;
    ...
    private getDomElements(): void {
        ...
        this.formStatusSelect = document.getElementById('form-status') as HTMLSelectElement;
    ...
    private setupAdminListeners(): void {
        ...
        this.resetCatalogBtn?.addEventListener('click', async () => {
    ...
    private async loadCatalog(): Promise<void> {
        ...
        try {
            ...
            if (config && config.projectId && !config.projectId.includes("REEMPLAZAR")) {
                ...
                this.db.collection("products").onSnapshot(async (snapshot: any) => {
                    if (snapshot.empty) {
                    ...
                    snapshot.forEach((doc: any) => {
                        list.push({ id: parseInt(doc.id), ...doc.data() } as Product);
                    });
    ...
    private async seedFirebaseCatalog(): Promise<void> {
        if (!this.db) return;
        try {
            const response = await fetch('products.json');
            if (response.ok) {
    ...
    private async loadLocalCatalogFallback(): Promise<void> {
        try {
    ...
    private updateDbStatusIndicator(isOnline: boolean): void {
        if (!this.dbStatusIndicator) return;
        if (isOnline) {
    ...
    private renderFilters(): void {
        ...
        this.statusFilterContainer.innerHTML = collections.map(col => {
            let count = 0;
            if (col === 'Todos') count = this.products.length;
            else if (col === 'Novedades') count = this.products.filter(p => p.status === 'Nuevo').length;
            else if (col === 'Promociones') count = this.products.filter(p => p.status === 'Promo').length;
    ...
    private filterProducts(): void {
        ...
        const filtered = this.products.filter(p => {
            ...
            if (this.currentStatus === 'Novedades') matchStatus = p.status === 'Nuevo';
            else if (this.currentStatus === 'Promociones') matchStatus = p.status === 'Promo';
    ...
    private async saveProduct(e: Event): Promise<void> {
        e.preventDefault();
        ...
        const status = this.formStatusSelect.value as any;
